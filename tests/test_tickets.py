...
# ---- POST /tickets/{id}/complete — завершение одним запросом ----


def _messages_payload() -> list[dict]:
    return [
...
async def test_complete_saves_everything_and_closes(client: AsyncClient) -> None:
    """Один запрос: закрытие + оценка + саммари + все сообщения диалога."""
    ticket = await _create(client)
    response = await client.post(
    ...
    assert body["status"] == "closed"
    assert body["closed_at"] is not None
    assert body["summary"] == "Пользователь спрашивал, как создать оферту."
    assert body["feedback"]["score"] == 5
    assert body["feedback"]["comment"] == "Помогли"
...
async def test_complete_builds_transcript_in_frontend_format(client: AsyncClient) -> None:
    """Поле `transcript` собирается из сообщений в формате парсера фронта."""
    ticket = await _create(client)
    await client.post(
        f"{API}/tickets/{ticket['id']}/complete",
        json={"messages": _messages_payload()},
    )
    fetched = (await client.get(f"{API}/tickets/{ticket['id']}")).json()
    assert fetched["transcript"] == (
        "Пользователь: Как создать оферту?\n\n"
        "Агент: Откройте раздел «Оферты» и нажмите «Создать»."
    )


async def test_complete_saves_structured_messages(client: AsyncClient) -> None:
    ticket = await _create(client)
    await client.post(
        f"{API}/tickets/{ticket['id']}/complete",
        json={"messages": _messages_payload()},
    )
    messages = (await client.get(f"{API}/tickets/{ticket['id']}/messages")).json()
    assert [(m["position"], m["role"]) for m in messages] == [
        (0, "user"),
        (1, "assistant"),
    ]
    assert messages[0]["content"] == "Как создать оферту?"
    assert messages[0]["ticket_id"] == ticket["id"]


async def test_complete_is_idempotent_for_closed_ticket(client: AsyncClient) -> None:
    """complete по уже закрытому обращению дозаписывает данные, не ломая историю."""
    ticket = await _create(client)
    closed = await client.post(f"{API}/tickets/{ticket['id']}/close", json={})
    closed_at = closed.json()["closed_at"]

    response = await client.post(
        f"{API}/tickets/{ticket['id']}/complete",
        json={"feedback": {"score": 4}, "messages": _messages_payload()},
    )
    assert response.status_code == 200
    assert response.json()["closed_at"] == closed_at

    events = (await client.get(f"{API}/tickets/{ticket['id']}/events")).json()
    assert [event["to_status"] for event in events] == ["created", "closed"]

    feedback = (await client.get(f"{API}/tickets/{ticket['id']}/feedback")).json()
    assert feedback["score"] == 4
    assert len((await client.get(f"{API}/tickets/{ticket['id']}/messages")).json()) == 2


async def test_repeated_complete_replaces_messages(client: AsyncClient) -> None:
    """Присланный снимок диалога авторитетный: старые реплики не дублируются."""
    ticket = await _create(client)
    await client.post(
        f"{API}/tickets/{ticket['id']}/complete",
        json={"messages": _messages_payload()},
    ...
    await client.post(
        f"{API}/tickets/{ticket['id']}/complete",
        json={"messages": [{"role": "user", "content": "Только вопрос"}]},
    )

    messages = (await client.get(f"{API}/tickets/{ticket['id']}/messages")).json()
    assert [(m["position"], m["content"]) for m in messages] == [(0, "Только вопрос")]

    fetched = (await client.get(f"{API}/tickets/{ticket['id']}")).json()
    assert fetched["transcript"] == "Пользователь: Только вопрос"


async def test_complete_without_optional_fields_closes_only(client: AsyncClient) -> None:
    ticket = await _create(client)
    response = await client.post(f"{API}/tickets/{ticket['id']}/complete", json={})
    body = response.json()
    assert body["status"] == "closed"
    assert body["feedback"] is None
    assert body["transcript"] is None
    assert (await client.get(f"{API}/tickets/{ticket['id']}/messages")).json() == []


async def test_complete_keeps_summary_when_not_resent(client: AsyncClient) -> None:
    """Саммари с эскалации не затирается, если complete пришёл без своего."""
    ticket = await _create(client)
    await client.post(
        f"{API}/tickets/{ticket['id']}/escalate",
        json={"reason": "agent_initiated", "summary": "Ответа в БЗ нет."},
    )
    await client.post(f"{API}/tickets/{ticket['id']}/complete", json={})

    fetched = (await client.get(f"{API}/tickets/{ticket['id']}")).json()
    assert fetched["status"] == "closed"
    assert fetched["summary"] == "Ответа в БЗ нет."


async def test_complete_overwrites_feedback_on_repeat(client: AsyncClient) -> None:
    ticket = await _create(client)
    await client.post(f"{API}/tickets/{ticket['id']}/complete", json={"feedback": {"score": 2}})
    await client.post(
        f"{API}/tickets/{ticket['id']}/complete",
        json={"feedback": {"score": 5, "comment": "Передумал"}},
    )
    feedback = (await client.get(f"{API}/tickets/{ticket['id']}/feedback")).json()
    assert feedback["score"] == 5
    assert feedback["comment"] == "Передумал"


async def test_complete_validates_score_and_role(client: AsyncClient) -> None:
    ticket = await _create(client)
    bad_score = await client.post(
        f"{API}/tickets/{ticket['id']}/complete", json={"feedback": {"score": 6}}
    )
...
async def test_complete_unknown_ticket_returns_404(client: AsyncClient) -> None:
    response = await client.post(
        f"{API}/tickets/11111111-1111-1111-1111-111111111111/complete",
        json={"feedback": {"score": 5}},
    )
    assert response.status_code == 404


async def test_messages_unknown_ticket_returns_404(client: AsyncClient) -> None:
    response = await client.get(f"{API}/tickets/11111111-1111-1111-1111-111111111111/messages")
    assert response.status_code == 404
