...
class EscalationReason(StrEnum):
    """Кто/что инициировал передачу обращения человеку (или закрытие агентом).

    Источник истины — ML-сервис (см. `docs/frontend.md` в репозитории ml):
    именно он присылает это значение в кадре `escalation`. Список должен
    оставаться зеркалом того файла — новое значение там означает 422 здесь,
    пока его не добавили.
    """
    ...
    CLARIFY_EXHAUSTED = "clarify_exhausted"  # три уточняющих вопроса подряд без результата
    CONTEXT_EXHAUSTED = "context_exhausted"  # диалог перерос контекстное окно модели
